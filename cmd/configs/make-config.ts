...
const fileName = async (type_file: string): Promise<string> => {
	try {
		...
		const config =
			MAKE_CONFIG[configKey as keyof typeof MAKE_CONFIG] ||
			(MAKE_CONFIG?.DEFAULT as unknown as { file_name: string });
...
const folderStructure = async (type_file: string): Promise<string> => {
	try {
		...
		const config =
			MAKE_CONFIG[configKey as keyof typeof MAKE_CONFIG] ||
			(MAKE_CONFIG?.DEFAULT as unknown as { folder_name: string });
...
const getFileDescription = async (type_file: string): Promise<string> => {
	try {
		...
		const config =
			MAKE_CONFIG[configKey as keyof typeof MAKE_CONFIG] ||
			(MAKE_CONFIG?.DEFAULT as unknown as { description: string });
...
const fileFormat = async (type_file: string): Promise<string> => {
	try {
		...
		const config =
			MAKE_CONFIG[configKey as keyof typeof MAKE_CONFIG] ||
			(MAKE_CONFIG?.DEFAULT as unknown as { extension: string });
