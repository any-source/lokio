...
export const updateFiles = async (projectDir: string, projectName: string): Promise<void> => {
    try {
        ...
        const updatePromises = Object.entries(filesReplacePackage).map(async ([filePath, fileConfig]) => {
            // More detailed validation
            if (!fileConfig || typeof fileConfig !== 'object') {
                throw new Error(`Invalid file configuration for ${filePath}: configuration is null or not an object`);
            }

            if (!Array.isArray(fileConfig.patterns)) {
                throw new Error(`Invalid file configuration for ${filePath}: patterns is not an array`);
            }
            ...
            try {
                ...
                for (const pattern of fileConfig.patterns) {
                    if (!validatePattern(pattern)) {
                        ...
                        throw new Error("Invalid pattern configuration: regex or replace is missing or invalid");
                    }

                    try {
                        const compiledRegex = new RegExp(pattern.regex, "g");
                        const replacedString = pattern.replace.replace(/\${name}/g, projectName);
